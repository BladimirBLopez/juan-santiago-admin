...
export default function EditorPrecios({
  ...
  return (
    <div className="rounded-xl border border-[#2a2a3d] bg-[#131319] p-4 flex items-center justify-between gap-3">
      <span className="text-sm text-[#e8eaed]">{label}</span>
      <div className="flex items-center gap-2">
        <span className="text-xs text-[#6b6b80]">Bs</span>
        <input
          type="number"
          value={monto}
          onChange={(e) => setMonto(e.target.value)}
          className="w-20 text-sm rounded-md border border-[#2a2a3d] bg-[#0a0a0f] text-[#e8eaed] px-2 py-1.5 outline-none focus:border-[#8b5cf6]/50"
        />
        <button
          onClick={guardar}
          disabled={guardando}
